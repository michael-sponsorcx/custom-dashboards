import { Stack, Text, Checkbox, Button, Alert, Accordion, ScrollArea, Group, Loader } from '@mantine/core';
import { useState, useEffect, useMemo } from 'react';
import { fetchCubeMetadata } from '../../../services/cube';

interface CommonFieldsSelectionProps {
  selectedViews: string[];
  onSave: (selectedFields: Array<{ fieldName: string; fieldTitle: string; fieldType: 'measure' | 'dimension' | 'date' }>) => void;
}
...
export function CommonFieldsSelection({ selectedViews, onSave }: CommonFieldsSelectionProps) {
  ...
  const loadCommonFields = async () => {
    ...
    try {
      ...
      const viewFieldsMap: Record<string, {
        measures: Set<string>;
      ...
      const findCommonFields = (fieldType: 'measures' | 'dimensions' | 'dates'): CommonField[] => {
        ...
        for (let i = 1; i < selectedViews.length; i++) {
          ...
          commonFieldNames = new Set(
            [...commonFieldNames].filter(name => viewFields.has(name))
          );
        }
  ...
  const totalCommonFields = commonFields.measures.length + commonFields.dimensions.length + commonFields.dates.length;

  if (loading) {
    return (
      <Stack align="center" gap="md" py="xl">
        <Loader />
        <Text size="sm" c="dimmed">Loading common fields...</Text>
      </Stack>
    );
  }
    ...
    return (
      <Stack gap="md" mt="md">
        <Alert color="yellow" title="No common fields found">
          The selected data sources do not have any fields in common.
  ...
  return (
    <Stack gap="md" mt="md">
      <Text size="sm" c="dimmed">
        Select the fields you want to make available as dashboard filters.
        Only fields that exist in all selected data sources are shown.
      </Text>
            ...
            <Accordion.Item value="measures">
              <Accordion.Control>
                <Group justify="space-between">
                  ...
                  <Text size="sm" c="dimmed">
                    {commonFields.measures.filter(m => selectedFields.has(m.name)).length} selected
                  </Text>
                </Group>
              </Accordion.Control>
              <Accordion.Panel>
                <Stack gap="xs">
                  <Group gap="xs">
                    <Button size="xs" variant="light" onClick={() => handleSelectAll(commonFields.measures)}>
                      Select All
                    </Button>
                    <Button size="xs" variant="light" onClick={() => handleDeselectAll(commonFields.measures)}>
                      Deselect All
                    </Button>
                  </Group>
            ...
            <Accordion.Item value="dimensions">
              <Accordion.Control>
                <Group justify="space-between">
                  ...
                  <Text size="sm" c="dimmed">
                    {commonFields.dimensions.filter(d => selectedFields.has(d.name)).length} selected
                  </Text>
                </Group>
              </Accordion.Control>
              <Accordion.Panel>
                <Stack gap="xs">
                  <Group gap="xs">
                    <Button size="xs" variant="light" onClick={() => handleSelectAll(commonFields.dimensions)}>
                      Select All
                    </Button>
                    <Button size="xs" variant="light" onClick={() => handleDeselectAll(commonFields.dimensions)}>
                      Deselect All
                    </Button>
                  </Group>
          ...
          {commonFields.dates.length > 0 && (
            <Accordion.Item value="dates">
              ...
              <Accordion.Panel>
                <Stack gap="xs">
                  <Group gap="xs">
                    <Button size="xs" variant="light" onClick={() => handleSelectAll(commonFields.dates)}>
                      Select All
                    </Button>
                    <Button size="xs" variant="light" onClick={() => handleDeselectAll(commonFields.dates)}>
                      Deselect All
                    </Button>
                  </Group>
